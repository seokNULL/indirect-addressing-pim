...
int main(int argc, char *argv[])
{
    ...
    if(cmd==0){
        ...
        set_info->q_size    = 0x0;
        set_info->r_size    = 0x0;
        clock_gettime(CLOCK_MONOTONIC, &start_PL2PL);
        
        if (ioctl(fd_dma, MEMCPY_PL2PL, set_info) < 0) {
            printf("ERROR DMA \n");
            return 0;
        }
        clock_gettime(CLOCK_MONOTONIC, &end_PL2PL);

        diff_PL2PL = BILLION * (end_PL2PL.tv_sec - start_PL2PL.tv_sec) + (end_PL2PL.tv_nsec - start_PL2PL.tv_nsec);
        printf("MEM_CPY PL --> PL execution time %llu nanoseconds %d %d\n", (long long unsigned int) diff_PL2PL, p_size, q_size);
        ...
        memcpy(PL_dstC_buf, PL_srcA_buf, srcA_size*sizeof(short));

        clock_gettime(CLOCK_MONOTONIC, &end_PS2PL_MEMCPY);

        diff_PS2PL_MEMCPY = BILLION * (end_PS2PL_MEMCPY.tv_sec - start_PS2PL_MEMCPY.tv_sec) + (end_PS2PL_MEMCPY.tv_nsec - start_PS2PL_MEMCPY.tv_nsec);
        printf("MEM_CPY FUNC PL --> PL execution time %llu nanoseconds %d %d\n", (long long unsigned int) diff_PS2PL_MEMCPY, p_size, q_size);

        printf("Correctness PL --> PL check!\n\n");
        for(int i=0; i<dstC_size; i++){ 
            if(PL_srcA_buf[i] != PL_dstC_buf[i]) {
                printf("Error PS_src[%d]=%d PL_dst[%d]=%d \n",i,PL_srcA_buf[i],i,PL_dstC_buf[i]);
                success = false;
                break;
            }
            //printf("Error PS_src[%d]=%d PL_dst[%d]=%d \n",i,PS_srcA_buf[i],i,PL_dstC_buf[i]);
        ...
        memcpy(PS_dstC_buf, PS_srcA_buf, srcA_size*sizeof(short));

        clock_gettime(CLOCK_MONOTONIC, &end_PS2PL_MEMCPY);

        diff_PS2PL_MEMCPY = BILLION * (end_PS2PL_MEMCPY.tv_sec - start_PS2PL_MEMCPY.tv_sec) + (end_PS2PL_MEMCPY.tv_nsec - start_PS2PL_MEMCPY.tv_nsec);
        printf("MEM_CPY FUNC PS --> PS execution time %llu nanoseconds %d %d\n", (long long unsigned int) diff_PS2PL_MEMCPY, p_size, q_size);

        printf("Correctness PS --> PS check!\n\n");
        for(int i=0; i<dstC_size; i++){ 
        ...
        if (!success)
            printf("Correctness PS --> PS check failed!\n\n");
        else
            printf("Correctness PS --> PS check done!\n\n");
    }     
    else{
        printf("CMD ERROR!\n");
    }
