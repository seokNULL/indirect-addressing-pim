...
static void print_help(const char *prog_name) {
  ...
  printf("      M : number of rows of matrix B and C. (default: 16)\n");
...
void bias_op_cpu(float *A, float *B, float *C, int M, int N, int opcode){
  //0: Addition
  //1: Subtraction
  //2: Multiplication
  switch (opcode) {
    case 0: for(int i = 0; i < M * N; i++){C[i] = A[i%N] + B[i];} break;
    case 1: for(int i = 0; i < M * N; i++){C[i] = A[i%N] - B[i];} break;
    case 2: for(int i = 0; i < M * N; i++){C[i] = A[i%N] * B[i];} break;
    default: break;
  }
}

void bias_op_pim(short *A, short *B, short *C, pim_args *pim_code, int M, int N, int opcode){
  //0: Addition
  //1: Subtraction
  //2: Multiplication
  ...
  switch (opcode) {
    ...
    // case 2: elewise_mul(pim_code); break;
      case 0: bias_add(pim_code); break;
      case 1: bias_sub(pim_code); break;
      case 2: bias_mul(pim_code); break;    
    default: break;
  }
}
...
int main(int argc, char *argv[])
{
  ...
  /* Allocate and initialize matrices on CPU and PIM*/
    ...
    //for validation
    bias_op_cpu(srcA, srcB, C_ans, M, N, opcode);
  ...
  printf("\n--------------------- Run Benchmark -----------------------\n");
    ...
    for (size_t i = 0; i < num_iterations; ++i) {
      ...
      switch (device) {
        case 0: bias_op_cpu(srcA, srcB, dstC, M, N, opcode); break;      
        case 1: bias_op_pim(srcA_pim, srcB_pim, dstC_pim, pim_code, M, N, opcode); break;
      }
